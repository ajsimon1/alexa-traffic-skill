"""
scrape traffic data off bing maps API, reformat to alexa flash brief json
API format
"""
import datetime
import json
...
url = "http://dev.virtualearth.net/REST/v1/Traffic/Incidents/" \
      "40.166281254206545,-74.78668212890625,40.74101426921151," \
      "-74.06707763671875/false?s=2,3,4&t=1,2,8,9&" \
      "key=ApZQPDiXe83SL22Po1FDQcQ3KwjmbjIL1_wZxRu0JbHoaRWNYFO5NXg11M7g_M8G"
...
def build_incident_report(parsed_json):
    ...
    for count, incident_dict in enumerate(parsed_json['resourceSets'][0]['resources']):
        ...
        main_text_str = "There is {} {} {}, its {} that the road is closed "\
                        "and its {} that the accident has been visually " \
                        "verified".format(severity,
                                          incident_type,
                                          description,
                                          road_closed,
                                          verified)
...
if __name__ == '__main__':
    exportable_json = json.dumps(build_final_list(build_total_count(parsed_json),
                            build_incident_report(parsed_json)), sort_keys=True)
    pp = pprint.PrettyPrinter(indent=4)
    pp.pprint(exportable_json)
